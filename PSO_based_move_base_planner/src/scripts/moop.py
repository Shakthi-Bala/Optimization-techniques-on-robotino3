...
class MOEPSO:
    ...
    def generate_particles(self):
        ...
        for i in range(self.num_particles):
            # Generate a mutant particle based on current position and velocity
    ...
    def mutate_particles(self, mutant_particles):
        # Perform mutation operation on mutant particles
    ...
    def adjust_particle_values(self):
        def adjust_particle_value(self, particles):
        # Ensure particles are within search space bounds
        for i in range(self.num_particles):
            for j in range(self.num_dimensions):
                lower_bound, upper_bound = self.search_space[j]
                # Adjust particle value if it exceeds search space bounds
...
class MOEPSONode:
    ...
    def run_moepso(self):
        ...
        for particle in self.moepso.particles:
            point = Point()
            point.x = particle[0]
            point.y = particle[1]
            point.z = particle[2]
            optimized_path.poses.append(point)
        self.path_publisher.publish(optimized_path)
        # Publish the best fitness value
        best_fitness = min(self.moepso.best_fitness, np.min(self.moepso.evaluate_fitness()))
        self.best_fitness_publisher.publish(best_fitness)

    def reconfigure_callback(self, config, level):
        parameters = [config.param1, config.param2, config.param3]  # Example parameters
        adjusted_parameters = self.adjust_parameters(parameters)
        rospy.loginfo("Adjusted parameters: {}".format(adjusted_parameters))
        return config


# Define the dynamic reconfigure ConfigType
ConfigType = type('ConfigType', (object,), {})

if __name__ == '__main__':
    try:
        moepso_node = MOEPSONode()
        rospy.spin()
